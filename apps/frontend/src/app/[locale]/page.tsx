...
import { getPosts, getProjects, getSkills, getExperiences } from "@/lib/api"
...
export default async function Portfolio({
  ...
  // Fetch dynamic data
  const [posts, projects, skills, experiences] = await Promise.all([
    getPosts({ locale, limit: 3, featured: true }),
    getProjects({ locale, limit: 6, featured: true }),
    getSkills({ locale, featured: true }),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-900 via-zinc-900 to-black text-white overflow-hidden">
              ...
              <div className="relative aspect-square rounded-xl overflow-hidden border border-zinc-800">
                ...
                <div className="absolute bottom-0 left-0 w-full p-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">{t("about.availabilityStatus")}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* Blog Section */}
      <BlogSection posts={posts.docs} locale={locale} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mt-16">
            <GlassmorphicCard>
              ...
              <div className="mt-8 pt-8 border-t border-zinc-800">
                <h4 className="text-lg font-medium mb-4">{t("contact.info.status")}</h4>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-green-500 animate-pulse"></div>
                  <span>{t("contact.info.statusMessage")}</span>
                </div>
              </div>
            </GlassmorphicCard>
        ...
        <div className="container flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-2">
              © {new Date().getFullYear()} Shine Kyaw Kyaw Aung. {t("footer.copyright")}
            </p>
          </div>
